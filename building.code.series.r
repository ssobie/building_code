#Script to generate time series of selected building code indices
#for the different GCM-BCCAQ pairs
...
get.variables.subset <- function(tas.data,tasmax.data,tasmin.data,
                                 ...
                                 uas.data,vas.data,dates,interval) { ##snd.data,
...
##  snd.mon <- get.data.subset(snd.data,dates,interval) 
  ...
  rv <- list(tas=tas.mon,tasmax=tasmax.mon,tasmin=tasmin.mon,
             ...
             uas=uas.mon,vas=vas.mon,dates=dates.mon) ##snd=snd.mon,
  return(rv)  
}

get.build.code.series <- function(build.code.fxn,full.variable.series,                                  
                                  past.var.subsets,start.var.subsets,middle.var.subsets,end.var.subsets) {

  series.variables <- build.code.fxn(full.variable.series)                                  
  ...
  past.means <- lapply(past.variables,mean)

  rx <- 2
  series.anom.variables <- mapply('-',series.variables,past.means,SIMPLIFY=FALSE)
  series.prct.variables <- lapply(mapply('/',series.anom.variables,past.means,SIMPLIFY=FALSE),function(x){x*100})
  series.anom.rounded <- lapply(lapply(series.anom.variables,round,rx),as.vector)
  series.prct.rounded <- lapply(lapply(series.prct.variables,round,rx),as.vector) 
  ...
  rv <- list(series=series.variables,anoms=series.anom.rounded,prct=series.prct.rounded)
  return(rv)
}
...
ashrae.humidity.parameters <- function(data.subsets) {
  ...
  dew.point.sub <- dew.point.temp(pas.sub$data,huss.sub$data)
  wet.bulb.sub  <- temp.wet.bulb(tas.sub$data,dew.point.sub,pas.sub$data,huss.sub$data)

  dew.point.low <- quant.fxn(dew.point.sub,as.factor(format(tas.sub$dates,'%Y')),pctl=0.004)
  hum.ratio.low <- quant.fxn(huss.sub$data*1000,as.factor(format(tas.sub$dates,'%Y')),pctl=0.004) ##In g/kg
  dew.mcdb.low <- conditional.mon.fxn(main=tas.sub$data,second=dew.point.sub,fac=as.factor(format(tas.sub$dates,'%Y')),pctl=0.004)

  db.mcwb.ann.99 <- conditional.mon.fxn(main=wet.bulb.sub,second=tasmax.sub$data,fac=as.factor(format(tas.sub$dates,'%Y')),pctl=0.99)

  wb.evap.ann.99 <- quant.fxn(wet.bulb.sub,as.factor(format(tas.sub$dates,'%Y')),pctl=0.99)

  wb.mcdb.ann.99 <- conditional.mon.fxn(main=tas.sub$data,second=wet.bulb.sub,fac=as.factor(format(tas.sub$dates,'%Y')),pctl=0.99)

  dew.point.990 <- quant.fxn(dew.point.sub,as.factor(format(tas.sub$dates,'%Y')),pctl=0.99)

  hum.ratio.990 <- quant.fxn(huss.sub$data*1000,as.factor(format(tas.sub$dates,'%Y')),pctl=0.990)

  dew.mcdb.990 <- conditional.mon.fxn(main=tas.sub$data,second=dew.point.sub,fac=as.factor(format(tas.sub$dates,'%Y')),pctl=0.990)
  
   rv <- list(dew.point.low,hum.ratio.low,dew.mcdb.low,
              db.mcwb.ann.99,wb.evap.ann.99,wb.mcdb.ann.99,
              dew.point.990,hum.ratio.990,dew.mcdb.990)
  return(rv)
...
for (scenario in scen.list) {
    ...
    for (g in seq_along(gcm.list)) {
        ...
        dates.data <- pr.data
        dates.data$data <- pr.data$time

        ##-----------------------------------------------------------------------------
        ##Monthly Parameters
        ##Full
        full.int <- '1951-2100'
        full.variable.series <- get.variables.subset(tas.data$data,tasmax.data$data,tasmin.data$data,
                                    psl.data$data,huss.data$data,pr.data$data,uas.data$data,vas.data$data,##snd.data,
                                    tasmax.data$time,full.int)
        ##Past
        past.int <- '1971-2000'
        ...
        ##------------------------------------------------------------------------------------------------
        ##ASHRAE Parameters
        ashrae.humidity <- get.build.code.series(ashrae.humidity.parameters,full.variable.series,
                                                 past.var.subsets,start.var.subsets,middle.var.subsets,end.var.subsets)
        ...
        ashrae.all.parameters <- ashrae.humidity$series
        if (g==1) {
...
##-----------------------------------------------------------------------------
        ...
        bc.all.parameters <- mapply(c,bc.quantiles,bc.mixed,bc.wind,SIMPLIFY=FALSE)
...
plot(apply(ashrae.matrix[[1]],2,mean),type='l',lwd=3,ylim=c(-35,0))
lines(apply(ashrae.matrix[[1]],2,quantile,0.1),lwd=3,col='gray')
lines(apply(ashrae.matrix[[1]],2,quantile,0.9),lwd=3,col='gray')
abline(h=0)
...
##        write.combined.table(ashrae.means,ashrae.10th,ashrae.90th,ashrae.all.names,write.combined.file)
        ...
        bc.means <- lapply(lapply(bc.matrix,function(x){apply(x,2,mean)}),round,1)

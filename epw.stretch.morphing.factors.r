...
##
gcm.1980s <- daily.aggregate(gcm.nc,var.name,
                        gcm=gcm,interval='1971-2000',
                        method=method,rlen=rlen,agg.fxn=agg.fxn)

gcm.2020s <- daily.aggregate(gcm.nc,var.name,
                        gcm=gcm,interval='2011-2040',
                        method=method,rlen=rlen,agg.fxn=agg.fxn)

gcm.2050s <- daily.aggregate(gcm.nc,var.name,
                        gcm=gcm,interval='2041-2070',
                        method=method,rlen=rlen,agg.fxn=agg.fxn)

gcm.2080s <- daily.aggregate(gcm.nc,var.name,
...
create.factor.file(gcm.nc,var.name,var.units,
                   alpha.2020s,

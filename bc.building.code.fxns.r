...
calc.return.periods <- function(data,dates,var.name,rperiod) {
  ...
  if (sum(is.na(data)) == length(data)) {
    ...
    ## To deal with fevd's less stable fitting algorithm        

    tryCatch({
        ts.fit <- suppressWarnings(fevd(ts.to.fit,type='GEV'))
        ts.old <- suppressWarnings(gev.fit(ts.to.fit,show=FALSE))
         ts.fit$results$par <- ts.old$mle
        names(ts.fit$results$par) <- c('location','scale','shape')
        ts.rp <- suppressWarnings(return.level(ts.fit,return.period=as.numeric(rperiod),make.plot=F))
    }, warning=function(war) {
      print('Warning from Return Period Fit')        
    }, error=function(err) {
      print('Error in RP Fitting')
      return(NA)
    }, finally={      
    })
